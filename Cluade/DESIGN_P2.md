...
> Phase 2 목표: UX 개선, 강사 기능 강화, 코드 품질 향상, 역할별 UI 분리
...
- ngrok 환경 대응: 모든 URLRequest에 `ngrok-skip-browser-warning: true` 헤더 추가

### 영향 범위
| 레이어 | 파일 |
|--------|------|
| Backend | `GlobalExceptionHandler.java`, `ErrorResponse.java`, 각 Service |
| iOS | `APIService.swift` (에러 파싱 로직, ngrok 헤더) |
...
### 목적
Admin이 역할을 변경했을 때 앱 UI가 즉시 반영되지 않는 문제를 해결하고, 역할별 탭 구성을 분리한다.
...
AdminUserListView → PATCH /admin/users/{id}/role
    → 성공 시 AuthManager.currentRole 갱신
...
| 역할 | 탭 구성 |
|------|---------|
| STUDENT | 홈, 내 강의, 포트폴리오, 마이페이지 |
| TEACHER | 홈, 내 회원, 강의 관리, 마이페이지 |
| ADMIN | 홈, 강의 관리(전체), 유저 관리, 마이페이지 |

**강사 제한:**
- 강의 상세 화면에서 수강 신청 버튼 숨김 (`isTeacher` 체크)
- 마이페이지에서 포트폴리오 대신 "내 회원" 섹션 표시

**관리자 기능:**
- 전체 강의 관리 (AdminCourseView) — 스와이프 삭제
- 유저 관리 — 역할 변경 + 계정 삭제

### 영향 범위
| 레이어 | 파일 |
|--------|------|
| iOS | `AuthManager.swift`, `ContentView.swift`, `AdminUserListView.swift`, `AdminCourseView.swift` (신규), `CourseDetailView.swift`, `MyPageView.swift` |
...
MyPageView → PhotosPicker (iOS 17)
    ...
    → 서버: /uploads/profile_{userId}_{timestamp}.jpg 저장
...
**7-4. 내 회원 탭 (TeacherStudentsView)**
- 강사 전용 탭으로 수강생 목록 표시
- 대시보드 API 활용하여 모든 강의의 수강생 통합 표시
- 학생 이름, 이메일, 진도율(%), 완료 레슨 수 표시
- 중복 학생 제거 (여러 강의 수강 시 1번만 표시)
...
| 레이어 | 파일 |
|--------|------|
| Backend | `TeacherController.java`, `TeacherService.java` |
| iOS | `TeacherCourseView.swift`, `TeacherStudentsView.swift` (신규), `TeacherCourseDetailView.swift` |
...
### 목적
관리자가 전체 강의와 유저를 통합 관리할 수 있게 한다.

### 설계

**8-1. 전체 강의 관리 (AdminCourseView)**
- 전체 강의 목록 조회 (GET /courses?page=0&size=100)
- 스와이프로 강의 삭제 (DELETE /teacher/courses/{id})
- 강의 제목, 카테고리, 강사명, 섹션/레슨 수 표시

**8-2. 유저 관리 (AdminUserListView)**
- 유저 목록 조회 (GET /admin/users)
- 역할 변경 (PATCH /admin/users/{id}/role)
- 계정 삭제 (DELETE /admin/users/{id})

**백엔드 엔드포인트 추가:**
| Method | Path | 설명 |
|--------|------|------|
| DELETE | `/admin/users/{userId}` | 유저 계정 삭제 |

### 영향 범위
| 레이어 | 파일 |
|--------|------|
| Backend | `AdminController.java` |
| iOS | `AdminCourseView.swift` (신규), `AdminUserListView.swift` |
...
### 목적
수강 신청 후 바로 강의를 시청할 수 있도록 흐름을 개선한다.

### 설계

**수강 신청 → 강의 이동 흐름:**
```
CourseDetailView → 수강 신청 버튼
    → POST /enrollments → 성공
    → 수강 성공 오버레이 표시
    → "강의 보러가기" 버튼 → CoursePlayerView로 navigationDestination 이동
```

**강사 수강 신청 버튼 숨김:**
- `@EnvironmentObject var authManager` 활용
- `authManager.isTeacher` == true이면 수강 신청 버튼 미표시

### 영향 범위
| 레이어 | 파일 |
|--------|------|
| iOS | `CourseDetailView.swift` |
...
| 테이블 | 필드 | 타입 | 설명 |
|--------|------|------|------|
| Member | profileImageUrl | VARCHAR | 프로필 이미지 경로 |
| Course | thumbnailUrl | VARCHAR | 강의 썸네일 경로 |
| LectureProgress | completed | BOOLEAN | 시청 완료 여부 |
